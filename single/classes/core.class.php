...
class BaiduMusic{
    ...
    */
	public function __construct($str){
		...
		// Validate API Status
		if(!self::$bdapi_open) ExtendedFunc::retError(1001);
	...
	private function chkInputStr($str)
	{
		...
		foreach ($paramChk as $paramName => $paramCfg) {
			if ($paramChk[$paramName]["force"]) { // A required parameter
				if (!isset($str[$paramName])) ExtendedFunc::retError(2002, $paramChk[$paramName]["error"]); // Really you didn't set it?
				if (isset($paramChk[$paramName]["filter"]) && (isset($paramChk[$paramName]["filter"]) && (!$filter_ret = $paramChk[$paramName]["filter"]["func"] /* Filter Type */ ? (ExtendedFunc::filterVar($paramChk[$paramName]["filter"]["type"] == FILTER_VALIDATE_INT ? (int)$str[$paramName] : $str[$paramName] /* Force convert string to integer when validating integer */, $paramChk[$paramName]["filter"]["type"], isset($paramChk[$paramName]["filter"]["option"]) /* If additional filter options defined */ ? $paramChk[$paramName]["filter"]["option"] : null) || (isset($paramChk[$paramName]["filter"]["include"]) /* If additional acceptable values array defined */ && in_array($str[$paramName], $paramChk[$paramName]["filter"]["include"]))) : in_array($str[$paramName], $paramChk[$paramName]["filter"]["include"]) /* We do not need filterVar(), in_array() only */))) ExtendedFunc::retError(2003, $paramChk[$paramName]["error"], 1);
			} else { // An optional parameter
				if (!isset($str[$paramName]) || (isset($paramChk[$paramName]["filter"]) && (!$filter_ret = $paramChk[$paramName]["filter"]["func"] ? (ExtendedFunc::filterVar($paramChk[$paramName]["filter"]["type"] == FILTER_VALIDATE_INT ? (int)$str[$paramName] : $str[$paramName], $paramChk[$paramName]["filter"]["type"], isset($paramChk[$paramName]["filter"]["option"]) ? $paramChk[$paramName]["filter"]["option"] : null) || (isset($paramChk[$paramName]["filter"]["include"]) && in_array($str[$paramName], $paramChk[$paramName]["filter"]["include"]))) : in_array($str[$paramName], $paramChk[$paramName]["filter"]["include"])))) $str[$paramName] = $paramChk[$paramName]["default"]; // Oooh you have put an invalid value, I won't blame you just overwrite it, sorry -.-
			}
	...
	private function execQuery($query_str){
		if(function_exists("curl_init")){
			...
			if($curl_errno = curl_errno($ch)){ // Error found
				ExtendedFunc::retError(1002, curl_strerror($curl_errno));
			}else{
				curl_close($ch);
				return array("isSucc" => 1, "result" => ExtendedFunc::tidyJson($ret)); // No error then output result
			}
		}
		ExtendedFunc::retError(1003);
	}
	...
	*/
	private function procData($data){
		if($data["isSucc"]){
			if(ExtendedFunc::is_json($data["result"]) && ($ret = json_decode($data["result"], 1)) && $ret["song_id"] == $this->querydata["id"]){ // Make sure song is exists
				...
				ExtendedFunc::retError(3001);
			}
		}
		ExtendedFunc::retError(1002, "Error Fetch Data from Server");
	}
	...
	*/
	private function procFileData($data){
		...
		ExtendedFunc::optResult($resp); // Then output it!
	}
